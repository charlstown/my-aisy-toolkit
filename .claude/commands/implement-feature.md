...
**Before dispatching anything**, look at the task's tag. If it is `· @human` (write/migration/deletion on PROD `facturito-pro` or its Neon/Vercel dashboard, the merge that triggers the deploy, go/no-go decisions, or reads only possible from a web dashboard):
