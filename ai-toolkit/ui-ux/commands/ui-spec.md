...
If the user passed an argument (e.g. `/ui-spec home`, `/ui-spec onboarding`), use that name. If not, ask in **a single line of text** before continuing:
...
- One screen per invocation — if the user asks for several, prioritize the first and suggest invoking `/ui-spec` again for the rest
