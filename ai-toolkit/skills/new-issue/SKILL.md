...
1. **Command argument** (text after `/aisy.new-issue`) → use it directly.
...
To work on the fix: /aisy.specify-feature → select #{number}
```
...
To plan the implementation: /aisy.specify-feature → select #{number}
```
