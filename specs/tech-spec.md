...
Written by Step 6 of `setup-ai.md`. Step 6 starts from the agent the user already confirmed in Step 1 ("confirmed agent") as its first candidate, rather than deriving candidacy purely from its own probe; the directory-existence and existing-file checks still run exactly as before, but now they only decide (a) whether the confirmed agent's candidacy survives and (b) whether the other agent joins as an additional candidate. It is written only when all of these hold: the run did not come from the launcher itself, the candidate's user-level directory actually exists, no file already sits at the destination, and the user answered yes to a single explicit question. If either check drops the confirmed agent, Step 6 says so explicitly in the same message as the offer (or the message replacing it), not only in the Wrap-up. Existing files are never opened, compared, or overwritten. Unlike Step 5, the Codex template is written verbatim, not translated — the launcher never comes from the catalog.
...
The global launcher (Step 6) is verified in the same run, checking three outcomes: it gets written on a yes, nothing is written on a no or a non-answer, and a second run neither asks again nor overwrites the existing file. Additionally, verify that the agent confirmed in Step 1 is always offered unless legitimately excluded (missing user-level directory or file already at destination — AC-01/AC-02), and that any such exclusion is announced in Step 6's own message, not only in the Wrap-up (AC-03).
