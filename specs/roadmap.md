...
|---|---|---|---|
| F1.1 | Populate `ai-toolkit/default/commands/` + `ai-toolkit/default/agents/` | — | Copied from the maintainer's local skills vault (`D:\MisProyectos\0_TEMPLATES\AI\SKILLS`), not authored from scratch (ADR-005) |
| F1.2 | `catalog.yaml` manifest | F1.1 | Declares the `default` profile: skill/agent list and source paths (ADR-003) |
| F1.3 | `setup-ai.md` installer | F1.2 | One-liner + copy-paste methods; always asks profile and target agent (ADR-004); fetches and writes, translating to Codex format at install time when needed (ADR-002) |
| F1.4 | README as product front | F1.3 | Presents the kit, both install methods, and the `default` profile catalog |
| F1.5 | Automated install verification check | F1.3 | Installs into a dummy/scratch folder and confirms every catalog file was copied unmodified |
...
```mermaid
flowchart LR
    VAULT[(Local skills vault<br/>D:\MisProyectos\0_TEMPLATES\AI\SKILLS)] -.copied manually.-> F1[F1.1 Populate ai-toolkit/default/]
...
- F1.5's automated check passes for a dummy-folder install via the one-liner method against Claude Code.
