...
- FR-00N: The exact literal syntax of the one-liner command/URL that the user pastes is [NEEDS CLARIFICATION: source material gives only a generic example format ("Fetch and follow the onboarding instructions from: <url>") used elsewhere in the spec set, not the literal one-liner text for setup-ai itself].
- FR-00N: The mechanism by which `setup-ai.md` itself is bootstrapped/fetched before it can begin fetching the manifest and other files is [NEEDS CLARIFICATION: not specified — e.g., is it fetched from a fixed URL, pasted in full, or fetched via some other entry point?].
- FR-00N: The exact wording/UX of the profile-selection and target-agent questions the installer asks the user is [NEEDS CLARIFICATION: only the fact that these questions must be asked is specified, not their phrasing or interaction format].
...
- [ ] What is the literal one-liner command/URL syntax the user pastes into their terminal or asks their agent to run (only a generic unrelated example format exists in the source material)?
- [ ] How is `setup-ai.md` itself fetched/bootstrapped before it can begin fetching the manifest and other files — is there a fixed entry-point URL, a pasted seed instruction, or another mechanism?
- [ ] What is the exact wording/UX of the profile-selection question and the target-agent question that the installer asks the user?
